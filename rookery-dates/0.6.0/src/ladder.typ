...
#let _assert-ladder(ladder) = {
  assert(
    ...
    message: "@rheo/rookery-dates: `ladder:` must be a dictionary with `transit:` "
  ...
  for k in ("transit", "terminal") {
    assert(
      ...
      message: "@rheo/rookery-dates: a ladder's `" + k + ":` must be an array of strings — got " + repr(ladder.at(k)),
    )
  }
  ...
  assert(
    both.len() == 0,
    message: "@rheo/rookery-dates: the stage name(s) "
      + both.join(", ")
