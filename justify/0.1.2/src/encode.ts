...
/** U+2060 — a word joiner: invisible, zero-width, "no break here". */
export const WORD_JOINER = "⁠";
/**
 * Characters the browser may break at INSIDE a word (UAX #14 classes B2/BA/SY:
 * em dash, en dash, horizontal bar, solidus, fraction slash). kp.ts's word model
 * splits on whitespace only, so it never plans a break at one of these — but the
 * browser will take one anyway if a line turns out a hair too wide, and the
 * fragment left behind is then justified across its few no-break spaces, blowing
 * one gap open to most of the column. Fencing each one in word joiners keeps the
 * browser to the breaks KP actually chose.
 */
const IN_WORD_BREAKS = /[—–―/⁄]/g;
...
function encodeWord(word: string, offsets: number[]): string {
  ...
  // Fence off the remaining in-word break opportunities. Done last: word joiners
  // are zero-width but not zero-length, so inserting them earlier would shift
  // the soft-hyphen offsets.
  return out.replace(IN_WORD_BREAKS, (c) => WORD_JOINER + c + WORD_JOINER);
}
