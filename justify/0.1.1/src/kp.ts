...
export interface BreakResult {
  ...
  fallback: boolean;
  /**
   * True if the optimum was only reachable by stretching some line past
   * `maxSpaceFrac`. The layout is still exact — every line fits the column — but
   * the author's stretch bound could not be honored everywhere.
   */
  relaxed: boolean;
}
...
/**
 * How the stretch bound is relaxed when the author's own `maxSpaceFrac` admits no
 * layout: raised in 10% steps until one does, so a relaxed paragraph is only as
 * loose as it has to be. Relaxing beats the greedy fallback outright — a loose
 * line still reproduces EXACTLY in the browser, whereas greedy only packs to the
 * natural width and cannot promise a line fits the column at all.
 *
 * Relaxation is needed far more often than it looks: on the HTML target the
 * encoded gaps are no-break spaces, which the browser stretches but never
 * shrinks, so `minSpaceFrac` is clamped to 1 and the usable band is just
 * `1 .. maxSpaceFrac`. Most columns admit no layout inside it.
 */
const RELAX_FACTOR = 1.1;
/** Stretch bound past which relaxation stops stepping and goes unbounded. */
const RELAX_CAP = 16;
 ...
 *
 * When the author's stretch bound (`maxSpaceFrac`) admits no layout, the bound
 * is relaxed in steps ({@link RELAX_STEPS}) and the tightest one that works is
 * used — a line stretched past the bound is still laid out exactly, so it beats
 * the greedy fallback, which only packs to the natural width and cannot promise
 * a line fits. Only when even unbounded stretch fails (an unbreakable segment
 * wider than the column) does it fall back to {@link greedyBreak}.
 */
export function breakParagraph(
  ...
  if (n === 0) {
    return { lines: [], breakJoins: [], hyphenBreaks: [], cost: 0, fallback: false, relaxed: false };
  }
  ...
  const strict = solve(s, targetWidth, cfg);
  if (strict) return strict;

  const from = Math.max(cfg.maxSpaceFrac, 1);
  for (let maxSpaceFrac = from * RELAX_FACTOR; maxSpaceFrac <= RELAX_CAP; maxSpaceFrac *= RELAX_FACTOR) {
    const relaxed = solve(s, targetWidth, { ...cfg, maxSpaceFrac });
    if (relaxed) return { ...relaxed, relaxed: true };
  }
  // Unbounded stretch: still exact, and still better than a plan the browser
  // cannot reproduce.
  const loose = solve(s, targetWidth, { ...cfg, maxSpaceFrac: Infinity });
  if (loose) return { ...loose, relaxed: true };

  // Nothing fits: some segment is wider than the column.
  return greedyBreak(s, targetWidth, cfg);
}

/**
 * One dynamic-programming pass at the given config. Returns null when no
 * feasible layout exists under `cfg` (the caller then relaxes or falls back).
 */
function solve(
  s: SegmentStream,
  targetWidth: number,
  ...
  const minSpace = Math.max(cfg.minSpaceFrac, 0) * s.spaceWidth;

  for (let i = n - 1; i >= 0; i--) {
    // Width of segments [i, end] at the tightest permitted spacing, accumulated
    // as `end` grows. Monotonic, so once it overflows the column no longer line
    // can be feasible either and the scan can stop — which keeps the relaxation
    // passes above cheap (the line-candidate scan is bounded by the number of
    // segments that fit on one line, not by the paragraph length).
    let tightest = 0;

    for (let end = i; end < n; end++) {
      tightest += s.segmentWidths[end];
      if (end > i && s.joins[end - 1] === "space") tightest += minSpace;
      if (end > i && tightest - targetWidth > 1e-6) break;
  ...
  if (!Number.isFinite(best[0]) || choice[0] === -1) return null;
  ...
  return { lines, breakJoins, hyphenBreaks, cost: best[0], fallback: false, relaxed: false };
}
...
export function greedyBreak(
  ...
  /**
   * Width of a line ending right after segment `k`, given the width of its
   * boxes and spaces. A `hyphen` join paints a hyphen glyph when a line ends
   * there, so that glyph counts towards the fit — without it a greedy line that
   * breaks at a hyphenation point is up to one hyphen too wide for the column,
   * and the encoded no-break spaces leave the browser no way to absorb it.
   */
  const endWidth = (width: number, k: number): number =>
    width + (k < n - 1 && s.joins[k] === "hyphen" ? s.hyphenWidth : 0);

  let start = 0;
  while (start < n) {
    // Grow the line to the furthest segment that still fits, remembering both
    // the furthest legal break (progress) and the furthest one that also fits
    // the column once its hyphen glyph is counted (quality).
    let width = s.segmentWidths[start];
    let end = start;
    let spaces = 0;
    let lastFit = -1; // furthest legal break whose line fits `targetWidth`.
    let lastLegal = -1; // furthest legal break, fitting or not.

    const noteCandidate = (k: number, w: number): void => {
      if (k !== n - 1 && !isBreakable(s.joins[k])) return;
      lastLegal = k;
      if (endWidth(w, k) <= targetWidth) lastFit = k;
    };

    // The start segment alone is always placed (guarantees progress).
    noteCandidate(start, width);
    ...
    while (cursor < n - 1) {
      ...
      if (width + addedWidth > targetWidth && lastLegal >= start) {
      ...
      noteCandidate(cursor, width);
    }

    // Prefer the furthest break that actually fits; else the furthest legal one
    // (a segment wider than the column has to overflow); else break right after
    // `end` (last resort, keeps the layout progressing).
    const breakAt = lastFit >= start ? lastFit : lastLegal >= start ? lastLegal : end;
  ...
  return { lines, breakJoins, hyphenBreaks, cost: Infinity, fallback: true, relaxed: false };
}
