...
test("in-word break opportunities are fenced off with word joiners", () => {
  // The word model splits on whitespace, so "good—as" is one box and KP never
  // plans a break at the dash — but the browser would take one, and justify the
  // fragment it left behind across a single gap.
  const out = encodeJustified({ words: ["good—as", "and/or"], breaks: [] });

  const expected =
    "good" + WORD_JOINER + "—" + WORD_JOINER + "as" +
    NBSP +
    "and" + WORD_JOINER + "/" + WORD_JOINER + "or";
  assert.equal(out, expected);
});

test("word joiners do not disturb soft-hyphen offsets", () => {
  const out = encodeJustified({
    words: ["co—operative"],
    breaks: [],
    hyphens: [{ word: 0, offset: 8 }], // inside "operative", after "co—opera".
  });

  assert.equal(
    out,
    "co" + WORD_JOINER + "—" + WORD_JOINER + "opera" + SOFT_HYPHEN + "tive",
  );
});
