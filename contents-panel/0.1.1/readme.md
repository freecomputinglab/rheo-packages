...
### Replayed sections are skipped, whatever the separator

**New in 0.1.1.** A `#window` replays a note's whole stored body onto the page
that windows it, headings and all. Those headings really are rendered there, so
a page-bracket test cannot tell them from the page's own sections, and without a
guard they are listed as though they were.

`separator: idea` has always carried that guard. The plain-selector path did
not, and off a rookery it never mattered, because nothing there replays
anything. It matters on a rookery whose sections happen to *be* headings.

Measured on waterline's ohrg.org — one idea per post, sections left as ordinary
headings, so `separator: heading` — where rookery renders a minted page's
Backlinks block as folded windows:
`/ideas/jensen-suther-novel-organism-form` listed five sections, of which four
belonged to the post that cited it. 15 of the site's 30 panelled pages were
wrong the same way.

Both paths now walk the same `<rookery-edge>` markers and skip anything nested
inside a window relative to the panel's own depth. A project with no such
markers — anything that is not a rookery — is unaffected: the walk finds no
markers, and the list is the bracket-filtered one it always was.
