...
```typst
#import "@rheo/rookery:0.1.1": rookery, idea, window
#show: rookery.with(
...
- `@idea:etal` — the terse form, but on its own it renders as a bare figure
  ...
  ```typst
  #import "@rheo/rookery:0.1.1": idea, window, link-to-page
  ...
  **Where it links:** `link-to-page` (above) goes to the note's own minted
  ...
  ```typst
  #import "@rheo/rookery:0.1.1": idea, window, link-to-anchor
...
```typst
#import "@rheo/rookery:0.1.1": ideas, note-href
#context {
  for e in ideas() {
    [#e.name — #e.text \ ]
  }
